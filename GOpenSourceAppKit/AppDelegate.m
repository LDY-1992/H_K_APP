...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [GosCommon sharedInstance].controlHandler = ^(GizWifiDevice *device, UIViewController *deviceListController) {
        //进入净智生活净化器
        if([device.productKey isEqualToString:PRODUCT_KEY_K19]){
        ...
        //进入摩纳净水器
        else if([device.productKey isEqualToString:PRODUCT_KEY_MO]){
            DeviceController_mo *devCtrl = [[DeviceController_mo alloc] initWithDevice:device] ;
            [deviceListController.navigationController pushViewController:devCtrl animated:YES];
        }
        //进入K18
        else if([device.productKey isEqualToString:PRODUCT_KEY_K18]){
            DeviceController_K18 *devCtrl = [[DeviceController_K18 alloc] initWithDevice:device] ;
            [deviceListController.navigationController pushViewController:devCtrl animated:YES];
        }
    };
    ...
    if ([GosCommon sharedInstance].cloudDomainDict.count > 0) {
        [GizWifiSDK startWithAppID:APP_ID appSecret:APP_SECRET specialProductKeys:@[PRODUCT_KEY_K18,PRODUCT_KEY_K19,PRODUCT_KEY_MO] cloudServiceInfo:[GosCommon sharedInstance].cloudDomainDict autoSetDeviceDomain:NO];
    } else {
        //设置服务器
